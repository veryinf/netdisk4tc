...
int wp_explorer(lua_State *l) {
    ...
    msg.hwnd = CreateWindowEx(0, L"WebBrowser", L"ÑéÖ¤", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, (rect.right - rect.left - width) / 2, (rect.bottom - rect.top - height) / 2, width, height, hParent, NULL, hInst, 0);
    tmp = lua_tostring(l, -1);
    sTmp = ctow(tmp);
